package net.azurewebsites.thehen101.bridgebuilder.util;
...
public class NahrFont {
	...
	private void drawer(String text, float x, float y, int color) {
		...
		for (int i = 0; i < text.length(); i++)
			if ((text.charAt(i) == '\u00A7') && (i + 1 < text.length())) {
				...
				if (colorCode < 16)
					try {
						int newColor = 0;// Minecraft.getMinecraft().fontRendererObj.colorCode[colorCode]; //TODO: reimplement this
	...
	private void drawTexturedModalRect(float x, float y, float u, float v, float width, float height) {
		...
		wr.addVertexWithUV(x + 0.0F, y + height, 0.0D, (u + 0.0F) * scale, (v + height) * scale);
		wr.addVertexWithUV(x + width, y + height, 0.0D, (u + width) * scale, (v + height) * scale);
		wr.addVertexWithUV(x + width, y + 0.0F, 0.0D, (u + width) * scale, (v + 0.0F) * scale);
		wr.addVertexWithUV(x + 0.0F, y + 0.0F, 0.0D, (u + 0.0F) * scale, (v + 0.0F) * scale);
		tessellator.draw();
	}
